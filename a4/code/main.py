...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        modelLR = LogisticRegression(C=100000)
        modelLR.fit(XMulti, yMulti)

        print("Logistic regression:")
        print("Training error %.3f" % utils.classification_error(modelLR.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(modelLR.predict(XMultiValid), yMultiValid))

        modelSM = LogisticRegression(C=100000, fit_intercept=False, multi_class='multinomial', solver='lbfgs')
        modelSM.fit(XMulti, yMulti)

        print("Softmax regression:")
        print("Training error %.3f" % utils.classification_error(modelSM.predict(XMulti), yMulti))
        print("Validation error %.3f" % utils.classification_error(modelSM.predict(XMultiValid), yMultiValid))